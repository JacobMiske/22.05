...
neutrons = 100; %neutrons = 1e6; %number of neutrons to simulate
...
waterMeanSquareCrowFlight = (waterMeanSquareCrowFlight).^2;
...
%% Mean distance travelled between birth and absorption adding between points
%Runs a find function for non-zeros, only accounts them for each column
[~,ii,v] = find(distanceBetweenN);
waterMeanPntOriginSumDistancePerN = accumarray(ii,v,[],@mean);
waterMeanPntOriginSumDistance = mean(waterMeanPntOriginSumDistancePerN);
 
%% Maximum distance of any neutron during flight (cm) adding between points
waterMaxFromOriginTotalDistancePerN = max(distanceBetweenN);
waterMaxFromOriginTotalDistance = max(waterMaxFromOriginTotalDistancePerN);
...
for i=1:neutrons
    ...
    for j=2:1:scatLimit
        ...
        %distance until next interaction with medium. Define distance in x,
        %y, and z by random incidence angles.
        distanceTravel=-log(rand())/graphiteSt;
        distanceX=distanceX+cosNx*distanceTravel; 
        distanceY=distanceY+cosNy*distanceTravel; 
        distanceZ=distanceZ+cosNz*distanceTravel;
        if i<=neutrons
...
%Take list of distances from origin, square them, and find the mean (*1/6)
%Run through ...X, ...Y, ...Z
for j = 1:size(graphiteX,2) %1:100 number of saved neutrons
    collisionCounter=0; %reset iterator
    for i = 2:size(graphiteX,1) %2:37 (happens to be max(col.) -1 (origin))
        if graphiteX(i,j) ~= 0
            collisionCounter=collisionCounter+1;
        end
    end
    %Save collisionCounter
    collisionsPerN(j) = (collisionCounter);
    graphiteMeanSquareCrowFlight(j) =  distanceFromOriginTracker(collisionCounter,j);
    
end
%Run square root and then mean all values
graphiteMeanSquareCrowFlight = (graphiteMeanSquareCrowFlight).^2;
graphiteMeanSquareCrowFlight = mean(graphiteMeanSquareCrowFlight);
graphiteMeanSquareCrowFlight=graphiteMeanSquareCrowFlight*1/6;
...
graphiteMeanPntOriginDistance = mean(graphiteMeanPntOriginDistancePerN);
...
for j = 1:size(D2OX,2) %1:100 number of saved neutrons
    collisionCounter=0; %reset iterator
    for i = 2:size(D2OX,1) %2:37 (happens to be max(col.) -1 (origin))
        if D2OX(i,j) ~= 0
            collisionCounter=collisionCounter+1;
        end
    end
    %Save collisionCounter
    collisionsPerN(j) = (collisionCounter);
    D2OMeanSquareCrowFlight(j) =  distanceFromOriginTracker(collisionCounter,j);
    
end
%Run square root and then mean all values
D2OMeanSquareCrowFlight = (D2OMeanSquareCrowFlight).^2;
D2OMeanSquareCrowFlight = mean(D2OMeanSquareCrowFlight);
...
D2OMeanPntOriginDistance = mean(D2OMeanPntOriginDistancePerN);
